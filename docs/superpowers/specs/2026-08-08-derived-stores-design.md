...
- **Авторегистрация:** конструктор сгенерированного **корневого** стора
  получает тело `{ StoreRootScope.register(this); }` — пишет в активное
  окружение: создание корня в тесте автоматически попадает в test-окружение,
  в приложении — в глобальный реестр. Некорневые сторы не меняются.
  `main.dart` не меняется вообще.
