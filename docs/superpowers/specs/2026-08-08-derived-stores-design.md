...
- `dispose()` для on-demand жизненного цикла; пользовательская логика перед
  диспозом — через `dispose()`, объявленный в impl: генератор вызовет
  `super.dispose()` первым (см. «Жизненный цикл»).
...
4. Валидации (понятные ошибки `InvalidGenerationSource`):
   ...
   - `dispose()` в derived impl с неподходящей сигнатурой (параметры,
     non-void) → ошибка; concrete `void dispose()` — легален: генератор
     вызывает его через `super.dispose()` первым;
   ...
   - коллизии имён с именем root-геттера, `*Raw`, `*$`; объявленный в impl
     `dispose` — не коллизия, а точка override (см. «Жизненный цикл»).
