...
3. Поведение signals после `dispose()` — **проверено эмпирически** (зонд на
   `signals` ^7.0.0):
   - **чтение** `.value` после dispose → возвращает последнее значение, печатает
     warning (`read after disposed`), НЕ бросает;
   - **запись** `.value` после dispose → бросает `SignalsWriteAfterDisposeError`;
   - **двойной dispose** → идемпотентен (`returnsNormally`).
   Контракт для derived-сторов: доступ к полям после `dispose()` не падает на
   чтении, но запись бросает; dispose можно вызывать повторно.
